...
namespace Tabalim.Addin.Model
{
    public class AlimTable : TabalimTable
    {
        ...
        public override HeaderOptions[] Headers
        {
            get
            {
                return new HeaderOptions[]
                {
                    ...
                    HeaderOptions.NoSubHeader( "Potencia\nDeman.\nTotal",  COLUMNWIDTH * 2.2d,2),
                    HeaderOptions.NoSubHeader( "Potencia\nDeman.\nTotal",  COLUMNWIDTH * 2.2d,2),
                    HeaderOptions.NoSubHeader( "Factor\nde\nPotencia",  COLUMNWIDTH * 2.4d,2),
                    HeaderOptions.NoSubHeader( "Fases", COLUMNWIDTH * 2.4d,3),
                    ...
                    HeaderOptions.NoSubHeader("Desconectador\nsin portafusibles", COLUMNWIDTH * 3.2d),
                    HeaderOptions.NoSubHeader("Fusible", COLUMNWIDTH *2.4d)
                };
            }
        }
        ...
        public Tuple<string, double>[] SubHeaders
        {
            get
            {
                return new Tuple<string, double>[]
                {
                    ...
                    new Tuple<string, double>( "HP",  Double.NaN),
                    null,
                    new Tuple<string, double>( "V.A.", Double.NaN),
                    new Tuple<string, double>( "V.A.", Double.NaN),
                    new Tuple<string, double>( "V.A.", Double.NaN),
        ...
        private void InitLines()
        {
            ...
            CellRange cellRange;
            row = startRow;
            var simRows = new int[] { 1, 2, 3, 4, 5, 19 };
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                this.Table.MergeCells(CellRange.Create(this.Table, row, 1, row, 5));
                //Canalización
                this.Write(lines[i].Canal, row, 19);
                //Nueva línea
                row++;
            }
        }
        public override void InitTitlesAndHeaders()
        {
            ...
            {
                CellRange.Create(this.Table, 1, 0, 4, 28),
                CellRange.Create(this.Table, 5, 0, 7, 2),
                CellRange.Create(this.Table, 5, 16, 7, 17),
            ...
            {
                //Application.DocumentManager.MdiActiveDocument.Editor.WriteMessage(String.Format("Columns:{0}-{1} Rows:{2}-{3}\n", cell.LeftColumn, cell.RightColumn, cell.TopRow, cell.BottomRow));
                this.Table.MergeCells(cell);
            });
