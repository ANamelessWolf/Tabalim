...
namespace Tabalim.Addin.Model
{
    ...
    public class TableroTable : TabalimTable
    {
        ...
        public override HeaderOptions[] Headers
        {
            get
            {
                return new HeaderOptions[]
                {
                    ...
                    new HeaderOptions( "Tierra", "#", COLUMNWIDTH * 2d),
                    new HeaderOptions( "Caida de tensión", "e%", COLUMNWIDTH * 3d),
        ...
        public override void SetTableSize()
        {
            ...
            numOfCols = 22 + this.Content.CmpColumns.Length;
            this.Table.SetSize(numOfRows, numOfCols);
        }

        private void CreateEmptyRows()
        {
            //Obtener polos totales
            int polos = 0;
            int i;
            if (this.Content.ImagenTablero.Split('_').Length == 2)
                polos = int.Parse(this.Content.ImagenTablero.Split('_')[1]);
            //Obtener polos faltantes
            var missing = Enumerable.Range(1, polos).Except(this.Content.CtoRows.SelectMany(x => x.Cto.Split(',')).Select(x => int.TryParse(x, out i) ? i : 0));
            //Agregar renglones vacios
            string countCommas = String.Join( "", Enumerable.Range(2, this.Content.CtoRows[0].Count.Count(x => x == ',')).Select(x => ","));
            this.Content.CtoRows = this.Content.CtoRows.Union(missing.Select(x => new CircuitoRow() { Cto = x.ToString(), Interruptor = "", Count = countCommas })).ToArray();
        }
        ...
        private void InitComponentes(ComponentColumn[] components)
        {
            ...
            for (int i = 0; i < components.Length; i++)
            {
                ...
                this.Write(Math.Round(components[i].VA).ToNumberFormat(), 11, 5 + i);
                ...
                this.Write(Math.Round(components[i].VATotales).ToNumberFormat(), rowCount, 5 + i);
            }
        }
        /// <summary>
