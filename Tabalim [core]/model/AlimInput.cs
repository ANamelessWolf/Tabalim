    ...
    public class AlimInput : IDatabaseMappable, ISQLiteParser
    {
        ...
        public InsertField[] GetInsertFields()
        {
            return new InsertField[]
            {
                ...
                this.CreateFieldAsNumber("is_charola", this.IsCharola),
                this.CreateFieldAsString("extra", this.Extra)
            };
        }
        /// <summary>
        /// Creates the linea.
        ...
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        internal Linea CreateLinea(List<TableroMock> tabs, List<BigMotor> motores, List<ExtraData> extras, List<DestinationRow> destinations, SQLite_Connector conn )
        {
            ...
            linea.Destination = new Destination(this.End, 
                ...
                tableros,
                motores.Where(x => destinations.Where(y => y.TypeId == 3 && y.AlimId == this.Id).Select(y => y.ConnId).Contains(x.Id)).Select(x => new BigAire(x)),
                extraData);
            ...
            linea.Extra = this.Extra;
            linea.IsCharola = this.IsCharola;
            //linea.GetNumber();
            return linea;
        }
        ...
        public void Parse(SelectionResult[] result)
        {
            try
            {
                ...
                this.IsCharola = result.GetValue<bool>("is_charola");
                this.Extra = result.GetString("extra");
            }
            catch (Exception exc)
            {
                throw exc;
            ...
            foreach (var val in input)
                switch (val.Key)
                {
                    ...
                    case "calibre":
                        ...
                        break;
                    case "is_charola":
                        this.IsCharola = (bool)val.Value;
                        break;
                    case "extra":
                        this.Extra = val.Value.ToString();
                        break;
                }
        }
