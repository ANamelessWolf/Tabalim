...
namespace Tabalim.Core.controller
{
    ...
    public class SQLite_Connector : IDisposable
    {
        ...
        public List<String> SelectTableColumns(String tablename)
        {
            String query = string.Format("PRAGMA table_info({0})", tablename);
            return this.Select(query).Select(x => x[1]).ToList();
        }
        public void AddColumnsToTable(String tablename, params InsertField[] columns)
        {
            String queryFormat = "ALTER TABLE {0} ADD COLUMN {1} {2} DEFAULT {3}";
            SQLiteCommand command;
            foreach(InsertField column in columns)
            {
                command = new SQLiteCommand();
                command.CommandText = String.Format(queryFormat, tablename, column.ColumnName, column.DataType.ToString(), column.Value.ToString());
                this.Query(command);
            }
        }
        /// <summary>
