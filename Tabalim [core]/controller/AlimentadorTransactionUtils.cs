...
namespace Tabalim.Core.controller
{
    public static partial class TransactionUtils
    {
            ...
            input.CreateAlimentadorTr(
                (Object qResult) =>
                {
                    ...
                    if ((Boolean)result[0] && alim != null)
                    {
                        ...
                        {
                            case 0:
                            case 7://Tablero solo insertar la conexión
                                //Tablero tab = destination.Cargas.FirstOrDefault();
                                //if (tab != null)
                                //    CreateAlimentadorDestinationTr(alim.Id, tab.Id, 1, alimoAddedTask);
                                CreateCargasAndDestinations(alim.Id, destination.Cargas, alimoAddedTask);
                                break;
                            ...
                            case 4://Varios motores y otras cargas
                                ...
                                break;
                            case 12://Un solo aire acondicionado
                            case 13://varios Aires acondicionados
                                CreateAiresAndDestinations(alim.Id, destination.Aires, alimoAddedTask);
                                break;
                            default://Capacitor, Transformador, sub-alimentador, generador
        ...
        public static void CreateAlimentadorTr(this AlimInput input, Destination destination, Action<Object, int> alimoAddedTask = null)
        {
            input.CreateAlimentadorTr(
                (Object qResult) =>
                {
        ...
        private static void CreateAiresAndDestinations(int id, IEnumerable<BigAire> aires, Action<object, int> connectionCreated)
        {
            SQLiteWrapper tr = new SQLiteWrapper(TabalimApp.AppDBPath)
            {
                TransactionTask = CreateAiresConnections,
                TaskCompleted = (Object result) => { connectionCreated(result, id); }
            };
            tr.Run(new Object[] { id, aires });
        }

        private static object CreateAiresConnections(SQLite_Connector conn, object input)
        {
            try
            {
                Object[] data = input as Object[];
                int alimId = (int)data[0];
                IEnumerable<BigAire> aires = (IEnumerable<BigAire>)data[1];
                Boolean flag;
                DestinationRow row;
                aires.ToList().ForEach(x =>
                {
                    flag = x.ToBigMotor().Create(conn, null);
                    if (flag)
                    {
        ...
        private static void CreateCargasAndDestinations(int alimId, IEnumerable<TableroMock> cargas, Action<object, int> connectionCreated)
        {
            SQLiteWrapper tr = new SQLiteWrapper(TabalimApp.AppDBPath)
            {
                TransactionTask = CreateCargasConnections,
                TaskCompleted = (Object result) => { connectionCreated(result, alimId); }
            };
            tr.Run(new Object[] { alimId, cargas });
        }

        private static object CreateCargasConnections(SQLite_Connector conn, object input)
        {
            try
            {
                Object[] data = input as Object[];
                int alimId = (int)data[0];
                IEnumerable<TableroMock> tableros = (IEnumerable<TableroMock>)data[1];
                Boolean flag;
                DestinationRow row;
                tableros.ToList().ForEach(x =>
                {
                    flag = x.Create(conn, null);
                    if (flag)
                    {
                        x.Id = (int)conn.SelectValue<long>("tableros_mock".SelectLastId(x.PrimaryKey));
                        row = new DestinationRow()
                        {
                            AlimId = alimId,
                            ConnId = x.Id,
                ...
                return tableros.Count(x => x.Id > 0) > 0;
            }
            catch(Exception exc)
            {
                return exc;
        ...
        /// <param name="alimoAddedTask">The alimo added task.</param>
        /// <exception cref="NotImplementedException"></exception>
        private static void CreateMotorAndCargasAndDestinations(int alimId, IEnumerable<TableroMock> cargas, IEnumerable<BigMotor> motors, Action<object,int> connectionCreated)
        {
            SQLiteWrapper tr = new SQLiteWrapper(TabalimApp.AppDBPath)
            {
        ...
        private static object CreateMotorsCargasConnections(SQLite_Connector conn, object input)
        {
            Object[] data = input as Object[];
            int alimId = (int)data[0];
            IEnumerable<TableroMock> cargas = (IEnumerable<TableroMock>)data[1];
            ...
            Object cargasResult = CreateCargasConnections(conn, new Object[] { alimId, cargas });
